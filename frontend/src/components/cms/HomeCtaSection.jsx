...
export default function HomeCtaSection({ sectionId, settings: s = {} }) {
  const title = s.title ?? 'Your next hire — or your next job — starts here.';
  const subtitle = s.subtitle ?? 'Free to sign up. No credit card required to browse listings or build your profile.';
  const btn1Text = s.btn1Text ?? 'Get started';
  const btn2Text = s.btn2Text ?? 'Browse jobs first';
